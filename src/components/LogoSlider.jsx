...
const LogoSlider = () => {
    ...
    return (
        <div className='bg-black overflow-hidden md:py-16 text-white relative'>
            {/* logo marquee */}
            <div className='flex gap-8 animate-marquee'>

                {
                    [logos,logos,logos].map((logoSet, logoIndex)=>(
                        <div key={logoIndex} className='flex shrink-0 min-w-full items-center justify-around px-4'>
                             {
                    logoSet.map((logo,index)=>(
                        <img key={`${logoIndex} - ${index}`} src={logo.src} alt={logo.alt} width={logo.width} height={logo.height} className='mx-8 object-contain shrink-0'/>
                    ))
                }
                        </div>
                    ))
